...
export class RedemptionService {
  ...
  async redeem(input: { cdk: unknown; session: unknown }): Promise<RedemptionResult> {
    ...
    try { const workspaceId = await this.requireWorkspaceSnapshot(); const normalized = this.requireNormalizedCdk(input.cdk); const codeHash = hashCdk(normalized, this.options.cdkHashSecret); await this.requirePossiblyUnused(codeHash); credentials = parseSingleSession(input.session); const claims = decodeSessionClaims(credentials.accessToken); const join = await this.options.joinClient.requestJoin(credentials, workspaceId); if (join.membership === 'member') { claimed = await this.claim(codeHash, claims.email, workspaceId); return await this.finishSuccess(claimed, 'already_member', claims.email); } if (!this.options.worker.isReadyForRedemptions) throw new DomainError('WORKER_UNAVAILABLE', 'Hệ thống duyệt hiện chưa sẵn sàng.'); const ticket = createAwaitableJob([claims.email]); this.options.queue.enqueue(ticket.job); const completion = await ticket.completion; this.assertAccepted(claims.email, completion); claimed = await this.claim(codeHash, claims.email, workspaceId); return await this.finishSuccess(claimed, 'accepted', claims.email); } catch (error) { return await this.failSafely(claimed, error); } finally { credentials?.clear(); credentials = null; }
  }
