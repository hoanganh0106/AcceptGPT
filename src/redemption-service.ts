...
export class RedemptionService {
  ...
  async redeem(input: { cdk: unknown; session: unknown }): Promise<RedemptionResult> {
    let credentials: SessionCredentials | null = null; let claimed: ClaimedCdk | null = null;
    try { const workspaceId = await this.requireWorkspaceSnapshot(); const normalized = this.requireNormalizedCdk(input.cdk); const codeHash = hashCdk(normalized, this.options.cdkHashSecret); await this.requirePossiblyUnused(codeHash); credentials = parseSingleSession(input.session); const claims = decodeSessionClaims(credentials.accessToken); const identity = await this.options.joinClient.validateSession(credentials, claims); claimed = await this.options.store.claimCdk({ codeHash, email: identity.email, workspaceId }); if (!claimed) throw new DomainError('CDK_INVALID_OR_USED', 'CDK không hợp lệ hoặc đã được sử dụng.'); const join = await this.options.joinClient.requestJoin(credentials, workspaceId); if (join.membership === 'member') return await this.finishSuccess(claimed, 'already_member', identity.email); if (!this.options.worker.isReadyForRedemptions) throw new DomainError('WORKER_UNAVAILABLE', 'Hệ thống duyệt hiện chưa sẵn sàng.'); const ticket = createAwaitableJob([identity.email]); this.options.queue.enqueue(ticket.job); const completion = await ticket.completion; this.assertAccepted(identity.email, completion); return await this.finishSuccess(claimed, 'accepted', identity.email); } catch (error) { return await this.failSafely(claimed, error); } finally { credentials?.clear(); credentials = null; }
  }
