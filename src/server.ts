...
function pageNumber(value: unknown, fallback: number, max: number): number { const parsed = typeof value === 'string' ? Number(value) : NaN; return Number.isInteger(parsed) && parsed >= 0 ? Math.min(parsed, max) : fallback; }
export function buildServer(config: ServerConfig, logger: Logger, dependencies: ServerDependencies): FastifyInstance {
  ...
  app.get('/api/admin/state', async (request, reply) => { const session = sessionFor(request, dependencies.adminAuth); if (!session) return reply.code(401).send({ code: 'UNAUTHORIZED', message: 'Cần đăng nhập.' }); const query = request.query as { q?: unknown; limit?: unknown; offset?: unknown }; const search = typeof query.q === 'string' ? query.q.trim().slice(0, 200) : ''; const limit = Math.max(1, pageNumber(query.limit, 50, 100)); const offset = pageNumber(query.offset, 0, 1_000_000); try { const workspaceId = await dependencies.cdkStore.getInviteWorkspaceId(); const history = await dependencies.cdkStore.listCdkHistory(limit, offset, search); return reply.header('cache-control', 'no-store').send({ csrfToken: session.csrfToken, workspaceId, history }); } catch { return reply.code(503).send({ code: 'SUPABASE_UNAVAILABLE', message: 'Dịch vụ tạm thời không khả dụng.' }); } });
  app.post('/api/admin/logout', async (request, reply) => { const session = sessionFor(request, dependencies.adminAuth); if (!session) return reply.code(401).send({ code: 'UNAUTHORIZED', message: 'Cần đăng nhập.' }); try { dependencies.adminAuth.assertMutation(session, request.headers['x-csrf-token'], request.headers.origin); } catch { return reply.code(403).send({ code: 'CSRF_REJECTED', message: 'Yêu cầu không hợp lệ.' }); } dependencies.adminAuth.logout(session); reply.clearCookie(ADMIN_COOKIE_NAME, { path: '/' }); return reply.send({ ok: true }); });
  ...
  app.post('/api/admin/cdks/delete', async (request, reply) => { if (!mutationSession(request, reply)) return; const body = request.body as { ids?: unknown; all?: unknown } | null; const ids = body?.all === true ? null : body?.ids; if (ids !== null && (!Array.isArray(ids) || ids.length < 1 || ids.length > 200 || !ids.every((id) => typeof id === 'string' && UUID.test(id)))) return reply.code(400).send({ code: 'INVALID_INPUT', message: 'Danh sách CDK không hợp lệ.' }); try { const deleted = await dependencies.cdkStore.deleteRemovableCdks(ids); return reply.header('cache-control', 'no-store').send({ deleted }); } catch { return reply.code(503).send({ code: 'SUPABASE_UNAVAILABLE', message: 'Dịch vụ tạm thời không khả dụng.' }); } });
  return app;
}
