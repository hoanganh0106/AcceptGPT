...
export class CdkIssuer {
  constructor(private readonly store: Pick<CdkStore, 'insertCdks'>, private readonly hashSecret: string) {}
  async issue(count: number): Promise<string[]> {
    ...
    for (let attempt = 0; attempt < 3; attempt++) { const batch = Array.from({ length: count }, () => generateCdk(this.hashSecret)); try { await this.store.insertCdks(batch.map((item) => ({ codeHash: item.codeHash, codePlain: item.plaintext.replaceAll('-', '') }))); return batch.map((item) => item.plaintext); } catch (error) { if (!isUniqueHashCollision(error) || attempt === 2) throw error; } }
