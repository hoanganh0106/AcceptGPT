...
export function buildServer(config: ServerConfig, logger: Logger, dependencies: ServerDependencies): FastifyInstance {
  ...
  void app.register(async (limited) => {
    await limited.register(rateLimit, {
      global: true,
      max: config.redeemRateLimitMax,
      timeWindow: config.rateLimitWindowMs,
      errorResponseBuilder: () => Object.defineProperty({ code: 'RATE_LIMITED', message: 'Bạn thao tác quá nhanh. Vui lòng thử lại sau.' }, 'statusCode', { value: 429 })
    });
