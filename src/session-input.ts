...
export function parseSingleSession(input: unknown): SessionCredentials {
  if (typeof input === 'string') { if (input.includes('\n') || input.includes('\r')) throw new DomainError('SESSION_INVALID', 'Chỉ được gửi một session.'); const token = input.trim(); if (!token || !tokenPattern.test(token)) throw new DomainError('SESSION_INVALID', 'Session không hợp lệ.'); return new MutableSessionCredentials(token); }
  if (!input || typeof input !== 'object' || Array.isArray(input)) throw new DomainError('SESSION_INVALID', 'Session không hợp lệ.');
  const values = input as Record<string, unknown>; const keys = ['accessToken', 'access_token', 'at']; const tokens = keys.map((key) => values[key]).filter((v): v is string => typeof v === 'string' && v.trim() !== '');
  if (tokens.length !== 1 || !tokenPattern.test(tokens[0].trim())) throw new DomainError('SESSION_INVALID', 'Session không hợp lệ.');
  const unknownKeys = Object.keys(values).filter((key) => !keys.includes(key)); if (unknownKeys.length) throw new DomainError('SESSION_INVALID', 'Session không hợp lệ.');
  return new MutableSessionCredentials(tokens[0].trim());
}
...
export function decodeSessionClaims(accessToken: string, now = Math.floor(Date.now() / 1000)): SessionClaims {
  ...
  if (!email || !accountId || typeof expiresAt !== 'number' || !Number.isFinite(expiresAt) || expiresAt <= now) throw new DomainError('SESSION_INVALID', 'Session không hợp lệ hoặc đã hết hạn.');
